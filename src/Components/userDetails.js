...
export default class UserDetails extends Component
{
    constructor(props) {
        ...
        this.gotoAdminView = this.gotoAdminView.bind(this);
    ...
    getFilesListForUser() {
        ...
        fetch(config.filesAPI + "getUserFiles?emailId=" + localStorage.getItem('token'),{
            method:"GET",
            mode:"cors",
            headers: {
    ...
    logoutUser = () => {
      ...
      gotoAdminView(ev) {
        ...
        if(localStorage.getItem('userRole').toString() == "1") {
    ...
    handleDeleteFile(ev, row) {
      ...
      async handleDownloadFile(ev, row) {
        ...
        window.open(config.awsCloudFront + 
    ...
    render()
    {
        ...
        <div>
          <Button className="btn btn-xs pull-right" bsStyle="success" onClick={this.gotoAdminView}>
          <img src="AdminAccount.png" width="70" height="50" alt=""/>
          </Button>
                </div>
            </div>
            </div>
